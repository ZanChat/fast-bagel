...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def to_tensor(self, sequence_status):
        ...
        if 'packed_vit_tokens' in sequence_status and len(sequence_status['packed_vit_tokens']) > 0:
        ...
        if 'packed_timesteps' in sequence_status and len(sequence_status['packed_timesteps']) > 0:
        ...
        if 'packed_label_ids' in sequence_status and len(sequence_status['packed_label_ids']) > 0:
        ...
        while True:
                    ...
                    if self.is_mandatory[group_index]:
                        while True:
                            ...
                            if num_tokens < self.max_num_tokens_per_sample:
                                sequence_status = self.pack_sequence(sample, sequence_status, no_cfg=True)
            ...
            if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                if len(buffer) < self.max_buffer_size and not sample_from_buffer:
                    ...
                    if data.get('packed_vit_tokens', None) is None or data.get('vit_token_seqlens', None) is None:
                        print(f'not Skipping vit none {data}')
                    if data.get('padded_images', None) is None:
                        print(f'Skipping vae none {data}')
                        # continue
                    yield data
                    sequence_status = self.set_sequence_status()
                    batch_data_indexes = []
            ...
            if sequence_status['curr'] >= self.expected_num_tokens:
                ...
                data['batch_data_indexes'] = batch_data_indexes
                if data.get('packed_vit_tokens', None) is None or data.get('vit_token_seqlens', None) is None:
                    print(f'Skipping vit none 2 {data}')
                if data.get('padded_images', None) is None:
                    print(f'Skipping vae none2 {data}')
                    # continue
    ...
    def pack_sequence(self, sample, sequence_status, no_cfg=False):
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                if item['enable_cfg'] == 1 and not no_cfg and random.random() < self.data_config.text_cond_dropout_prob:
                    continue
                ...
                if item['enable_cfg'] == 1 and not no_cfg and random.random() < self.data_config.vit_cond_dropout_prob:
                    curr_rope_id += 1
                    # print(f'drop vit')
                    continue
                ...
                image_tensor = image_tensor_list.pop(0)
                if item['enable_cfg'] == 1 and not no_cfg and random.random() < self.data_config.vae_cond_dropout_prob:
