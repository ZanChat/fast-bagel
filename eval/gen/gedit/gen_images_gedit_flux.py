...
def setup_models(model_path, device=0, use_heun=False):
    """
    ...
    tokenizer_path = "/some/path/bagelrev/BAGEL/BAGEL-7B-MoT/"
    model_variant = None

    vae_model, vae_config = load_ae(local_path=os.path.join(tokenizer_path, "ae.safetensors"))
    print(
        f'load vae model {vae_model.decoder.norm_out.weight.data.dtype} {vae_model.decoder.norm_out.weight.data.device}')
    ...
    print(f'Loading model with use_heun: {use_heun}')
    ...
    model = dispatch_model(model, device_map=device_map, offload_dir="/tmp/offload", offload_buffers=False,
                           force_hooks=True)
...
def process_dataset(
    model, vae_transform, vit_transform,
    output_dir, cfg_text_scale=4.0, cfg_img_scale=1.5, num_timesteps=50, timestep_shift=4.0,
    cfg_type="serial_text_img", num_samples=None, shard_id=0, total_shards=1, use_think=False, device='cuda', ds_path=None,
):
    """
    ...
    # dataset = load_dataset(ds_path, cache_dir='./cache')['train']
    dataset = load_from_disk(ds_path).filter(lambda x: x['instruction_language'] in ['en'])
...
def main():
    ...
    parser.add_argument("--device", type=str, default='cuda', help="GPU device ID")
    ...
    parser.add_argument("--timestep_shift", type=float, default=4.0, help="Timestep shift")
    ...
    parser.add_argument("--num_timesteps", type=int, default=28,
                        help="Number of timesteps to process (None for all)")
    ...
    parser.add_argument("--use_heun", action='store_true',
                        help="Whether enable heun solver")
    
    args = parser.parse_args()

    pipe = FluxKontextPipeline.from_pretrained("/some/path/FLUX.1-Kontext-dev",
    ...
    # Process dataset
    process_dataset(
        pipe, vae_transform, vit_transform,
        args.output_dir, 
