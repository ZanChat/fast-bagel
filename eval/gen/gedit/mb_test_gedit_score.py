...
def process_single_item(item, vie_score, max_retries=10000):
    ...
    key = f'{item["img_id"]}{item["turn_index"]}'
    instruction_language = 'en'
    intersection_exist = False
    sample_prefix = key
    save_path_fullset_source_image = f"{save_path}/fullset/{group_name}/{instruction_language}/{key}_SRCIMG_MASK.png"
    save_path_fullset_result_image = f"{save_path}/fullset/{group_name}/{instruction_language}/{key}_MASK.png"
...
if __name__ == "__main__":
    ...
    dataset = load_dataset('parquet', data_files=os.path.join(args.ds_path, '*.parquet'),
                           split='train[:5%]',
                           cache_dir=os.path.join(args.ds_path, '../cache'), num_proc=32)
