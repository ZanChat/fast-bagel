...
def process_single_item(item, vie_score, max_retries=10000):
    ...
    if not megfile.smart_exists(save_path_fullset_source_image):
        save_path_fullset_source_image = f"/some/path/gedit-bench-good700/results/gen_image/fullset/{group_name}/{instruction_language}/{key}_SRCIMG.png"
        save_path_fullset_result_image = f"{save_path}/fullset/{group_name}/{instruction_language}/{key}.webp"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--groups", type=str, required=True, nargs='+')
    ...
    dataset = load_from_disk(args.ds_path).filter(lambda x: x['instruction_language'] in ['en'])
    # dataset = load_dataset(args.ds_path)['train']

    dataset = dataset.remove_columns(['input_image_raw', 'input_image'])
    ...
    for model_name in evaluate_group:
        ...
        for group_name in args.groups:
            if group_name not in GROUPS:
                print(f"Warning: Group {group_name} not recognized, skipping.")
                continue
            group_csv_list = []
            group_dataset_list = []  
            ...
            with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                ...
                for i, item in enumerate(group_dataset_list):
                    ...
                    if not megfile.smart_exists(save_path_fullset_result_image) or not megfile.smart_exists(save_path_fullset_source_image):
                        # print(f"Skipping {sample_prefix}: Source or edited image does not exist, try webp")
                        save_path_fullset_source_image = f"/some/path/gedit-bench-good700/results/gen_image/fullset/{group_name}/{instruction_language}/{key}_SRCIMG.png"
                        save_path_fullset_result_image = f"{save_path}/fullset/{group_name}/{instruction_language}/{key}.webp"
                        if not megfile.smart_exists(save_path_fullset_result_image) or not megfile.smart_exists(
                                save_path_fullset_source_image):
                            print(f"Skipping {save_path_fullset_source_image}: Source or edited image does not exist, and webp not found")
                            continue
