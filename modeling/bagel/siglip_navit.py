...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.patch_embedding = self.convert_conv2d_to_linear(self.config, meta=False)
    ...
    def convert_conv2d_to_linear(self, config, meta=False):
        ...
        W = patch_embedding.weight.permute(0, 2, 3, 1).reshape(
            self.embed_dim, config.num_channels * self.patch_size ** 2
        )
        linear_patch_embedding.weight.data = W
        linear_patch_embedding.bias.data = patch_embedding.bias.data
        return linear_patch_embedding
...
class SiglipVisionModel(SiglipPreTrainedModel):
    ...
    def forward(
        ...
        # print(f'enter vit forward {datetime.datetime.now()}')
