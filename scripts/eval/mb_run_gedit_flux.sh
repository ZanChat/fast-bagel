...
OUTPUT_DIR="/some/path/mb-gedit-bench-flux-s28/results"
...
for ((i=0; i<$N_GPU; i++)); do
    CUDA_VISIBLE_DEVICES=$i nohup python3 eval/gen/gedit/mb_gen_images_gedit_flux.py --model_path "$MODEL_PATH" --ds_path $ds_path  --output_dir "$GEN_DIR"  --shard_id $i --total_shards "$N_GPU" --num_timesteps $NUM_STEPS  2>&1 | tee "$LOG_DIR"/request_$(($N_GPU + i)).log &
done
