...
OUTPUT_DIR="/some/path/bagelrev/gedit-bench-${MODEL_NAME}-s${NUM_STEPS}/results"
...
for ((i=0; i<$N_GPU; i++)); do
    CUDA_VISIBLE_DEVICES=$i nohup python3 eval/gen/gedit/gen_images_gedit.py --model_path "$MODEL_PATH" --ds_path $ds_path  --output_dir "$GEN_DIR"  --shard_id $i --total_shards "$N_GPU" --num_timesteps $NUM_STEPS --cfg_img_scale 1.5 --cfg_text_scale 4.0 --timestep_shift 3.0 --device cuda  2>&1 | tee "$LOG_DIR"/request_$(($N_GPU + i)).log &
