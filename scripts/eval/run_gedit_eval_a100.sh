...
MODEL_PATH="/some/path/good1500"
OUTPUT_DIR="/some/path/gedit-bench-good1500-4/results"
...
#    CUDA_VISIBLE_DEVICES=$i nohup python3 eval/gen/gedit/gen_images_gedit.py --model_path "$MODEL_PATH" --ds_path $ds_path  --output_dir "$GEN_DIR"  --shard_id $i --total_shards "$N_GPU" --num_timesteps 30 --cfg_img_scale 2.0 --cfg_text_scale 4.0 --timestep_shift 4.0 --device cuda  2>&1 | tee "$LOG_DIR"/request_$(($N_GPU + i)).log &
#done
...
#CUDA_VISIBLE_DEVICES=2 python test_gedit_score.py --save_path "$OUTPUT_DIR" --ds_path $ds_path --azure_endpoint "$AZURE_ENDPOINT" --gpt_keys "$AZURE_OPENAI_KEY" --backbone "qwen25vl" --max_workers "$N_GPT_PARALLEL" --groups "subject-replace" "text_change" &
#CUDA_VISIBLE_DEVICES=3 python test_gedit_score.py --save_path "$OUTPUT_DIR" --ds_path $ds_path --azure_endpoint "$AZURE_ENDPOINT" --gpt_keys "$AZURE_OPENAI_KEY" --backbone "qwen25vl" --max_workers "$N_GPT_PARALLEL" --groups "motion_change" "subject-remove" "tone_transfer" &
