...
for ((i=0; i<$N_GPU; i++)); do
    CUDA_VISIBLE_DEVICES=1 nohup python3 eval/gen/gedit/gen_images_gedit_flux.py --model_path "$MODEL_PATH" --ds_path $ds_path  --output_dir "$GEN_DIR"  --shard_id $i --total_shards "$N_GPU" --num_timesteps $NUM_STEPS  2>&1 | tee "$LOG_DIR"/request_$(($N_GPU + i)).log &
done
