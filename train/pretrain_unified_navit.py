...
class ModelArguments:
    ...
    llm_variant: str = field(
        default='',
        metadata={"help": "pretrained variant."}
    )
    ...
    start_step: int = field(
        default=0,
        metadata={"help": "train start step."}
    )
    vit_rope: bool = field(
        default=False,
...
class TrainingArguments:
    ...
    acc_step: int = field(
        default=1,
        metadata={"help": "Accumulate gradients over this many steps before optimizer step."}
    )
...
def main():
    ...
    torch.distributed.constants.default_pg_nccl_timeout = datetime.timedelta(seconds=360000)
    dist.init_process_group("nccl", timeout=datetime.timedelta(seconds=360000))
    ...
    if training_args.auto_resume:
        ...
        print(f'get resume from is {resume_from}')
    ...
    # bugfix pytorch cudaMallocAsync
    torch.backends.cuda.matmul.allow_tf32 = True
    torch.backends.cuda.mem_efficient_fp16 = False
    torch.cuda.empty_cache()


    model_config = BagelConfig.from_pretrained(
        model_args.llm_path,
    ...
    model_config.llm_config.freeze_und = training_args.freeze_und
    model_config.llm_config.layer_module = model_args.layer_module
    model_config.llm_config.qk_norm = model_args.llm_qk_norm
    model_config.llm_config.tie_word_embeddings = model_args.tie_word_embeddings

    if training_args.visual_und:
        model_config.vit_config.rope = model_args.vit_rope

    # Setup model:
    model = Bagel.from_pretrained(model_args.llm_path,
                                  config=model_config,
                                  variant=model_args.llm_variant if len(model_args.llm_variant) > 0 else None,
                                  # visual_und=training_args.visual_und,
                                  # visual_gen=training_args.visual_gen,
                                  # latent_patch_size=model_args.latent_patch_size,
                                  # max_latent_size=model_args.max_latent_size,
    ...
    # llm_config = Qwen2Config.from_json_file(os.path.join(model_args.llm_path, "llm_config.json"))
    # llm_config.layer_module = model_args.layer_module
    # llm_config.qk_norm = model_args.llm_qk_norm
    # llm_config.tie_word_embeddings = model_args.tie_word_embeddings
    # llm_config.freeze_und = training_args.freeze_und
    # language_model = Qwen2ForCausalLM.from_pretrained(model_args.llm_path)
    ...
    #     print(f'test save ...')
    #     model.save_pretrained(training_args.checkpoint_dir)
    #     print(f'after save')
    ...
    tokenizer = Qwen2Tokenizer.from_pretrained(model_args.tokenizer_path)
    ...
    if training_args.freeze_und and training_args.visual_und:
        print(f'freeze vit related')
        model.connector.eval()
        for param in model.connector.parameters():
            param.requires_grad = False
        model.vit_pos_embed.eval()
        for param in model.vit_pos_embed.parameters():
            param.requires_grad = False
    ...
    if resume_model_only:
        ...
        _, scheduler, train_step, data_status = FSDPCheckpoint.try_load_train_state(
    ...
    if model_args.start_step > train_step:
        train_step = model_args.start_step

    optimizer.zero_grad()
    # train loop
    start_time = time.time()
    logger.info(f"Training for {training_args.total_steps} steps, starting at {train_step}...")
    for curr_step, data in tqdm(enumerate(train_loader, start=0), initial=0, total=training_args.total_steps):
        model.train_step = curr_step
        ...
        # optimizer.zero_grad()
        loss /= training_args.acc_step
        loss.backward()

        if curr_step > 0 and (curr_step % training_args.acc_step ==0 or curr_step == training_args.total_steps - 1):
        ...
        if curr_step % training_args.log_every == 0:
            ...
            for key, value in loss_dict.items():
                ...
                message += f"Train Loss {key}: {avg_loss:.6f}, "
        ...
        if curr_step > train_step  and curr_step % training_args.save_every == 0:
            if dist.get_rank() == 0:
